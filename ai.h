#include <iostream>
#include <string>
#include <fstream>

using namespace std;

#ifndef AI
#define AI

class ai{
	protected:
	   string ai_name;
	   int banker_bet;
	   int bet;
	   int capital;
	   int cycle1324;
	   int fibidx;
	   int paroli_cycle;
	public:
	   string get_ai_name(); // this one is fine
	   virtual void set_bet_type(int*, int, int) = 0; //pure virtual so that in every ai class it is redefined
	   int get_bet_type(); // this one doesnt need to be redefined in every class
	   
	   
	   
	   int bet10();
	   int bet25();
	   int doubleup(int, int*, int);
	   int one_three_two_four(int*, int);
	   int fibonacci(int*, int);
	   int dalembert(int*, int, int);
	   int paroli(int*, int);
	   int count_bet(int);

	   void set_bet(int, int*, int, int); //if i set this up in this base class i dont have to redefine it
	   int get_bet(); // doesnt need to be redefined
	   void add_to_capital(int); //doesnt need to be redefined in every class
	   void sub_from_capital(int); //doesnt need to be redefined in every class
	   int get_capital(); //doesnt need to be redefined in every class
	   virtual void output_to_file(int) = 0;
	   void stop_betting();


};

#endif
